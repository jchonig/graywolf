...
| Route overlays | Operator-uploaded route lines (e.g. a Ride With GPS course) drawn on the live map as a reference polyline. Uploaded on `MapsSettings.svelte` ("Route overlays" card): a hidden `<input type=file accept=".gpx,.kml,.geojson,.json">` feeds `route-import.js` `parseRouteFile(name, text, DomParser=globalThis.DOMParser)` — GPX/KML go through **`@tmcw/togeojson`** (`gpx()`/`kml()`, needs a DOM; the browser's native `DOMParser` in prod, `@xmldom/xmldom` injected as the 3rd arg in `route-import.test.js`), GeoJSON via `JSON.parse`; the result is normalized to a `FeatureCollection` of `LineString`s only (MultiLineString split, waypoints/elevation/styling dropped). Dense tracks (>2000 pts) get an inline Visvalingam-Whyatt pass (`simplifyRoute`) -- area-based elimination, not Ramer-Douglas-Peucker, per reviewer feedback that it better matches expected simplified-track shape. Persisted **server-side** and shared across devices, same model as fixed points: `GET/POST/PUT/DELETE /api/map-routes` (handlers `pkg/webapi/map_routes.go` — `create`/`update` are hand-rolled, not the generic helpers, so the body is `http.MaxBytesReader`-capped at 8 MiB; DTO `dto.MapRouteRequest` in `pkg/webapi/dto/map_route.go` validates the GeoJSON: line geometry only, coords on-globe, ≤6 MiB / ≤200k vertices; model `configstore.MapRoute`, `AutoMigrate` only). Client trio mirrors fixed points: pure `map-routes-api.js` (`routeFromApi`/`routeToApi`), singleton `map-routes-store.svelte.js` (`load`/`add`/`update`/`remove`), GL line layer `layers/map-routes.js` (one `type:line` source, `line-color: ['get','color']`). `LiveMapV2.svelte` mounts the layer after trails, loads the store on map mount, and renders a "Routes" section in the layers panel **only when ≥1 route exists**: a master `layerToggles.routes` checkbox plus one per-route checkbox whose state lives in `layerToggles.routeVisibility` (id→bool map, absent = visible) — both persisted in `gw_map_layer_toggles`. Per-route color/name are edited on the settings card (`<input type=color>` + text, commit on change/blur → `store.update` → PUT). Delete never emits an APRS object-kill (routes are never transmitted). Unit tests: `route-import.test.js`, `map-routes-api.test.js`, `pkg/webapi/map_routes_test.go`, `pkg/configstore/map_routes_test.go`. |
